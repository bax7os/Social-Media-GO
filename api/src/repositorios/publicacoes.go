...
func (repositorio Publicacoes) Buscar(ID uint64) ([]modelos.Publicacao, error) {
	linhas, erro := repositorio.db.Query(
		"select distinct p.*, u.nick from publicacoes p inner join usuarios u on u.id = p.autor_id inner join seguidores s on p.autor_id = s.usuario_id where p.id = ? or s.seguidor_id = ? order by 1 desc",
		ID, ID,
	)
	if erro != nil {
		return nil, erro
